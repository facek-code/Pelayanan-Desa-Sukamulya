															...
															<tbody>
																<?php foreach ($cdesa as $item): ?>
																	<tr>
																		<td><?= $item['no']?></td>
																		<td nowrap>
																			<a href="<?= site_url("cdesa/rincian/".$item["id"])?>" class="btn bg-purple btn-flat btn-sm"  title="Rincian"><i class="fa fa-bars"></i></a>
																			<a href="<?= site_url("cdesa/create_bidang/".$item["id"])?>" class="btn bg-green btn-flat btn-sm"  title="Tambah Data"><i class="fa fa-plus"></i></a>
																			<a href="<?= site_url("cdesa/create/edit/".$item["id"])?>" class="btn bg-yellow btn-flat btn-sm"  title="Ubah Data"><i class="fa fa-edit"></i></a>
											...
											<div class="row">
												<div class="col-sm-6">
													<div class="dataTables_length">
														<form id="paging" action="<?= site_url("cdesa")?>" method="post" class="form-horizontal">
															<label>
																Tampilkan
													...
													<div class="dataTables_paginate paging_simple_numbers">
														<ul class="pagination">
															<?php if ($paging->start_link): ?>
																<li><a href="<?= site_url("cdesa/index/$paging->start_link/$o")?>" aria-label="First"><span aria-hidden="true">Awal</span></a></li>
															<?php endif; ?>
															<?php if ($paging->prev): ?>
																<li><a href="<?= site_url("cdesa/index/$paging->prev/$o")?>" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
															<?php endif; ?>
															<?php for ($i=$paging->start_link;$i<=$paging->end_link;$i++): ?>
																<li <?=jecho($p, $i, "class='active'")?>><a href="<?= site_url("data_persil/index/$kat/$mana/$i/$o")?>"><?= $i?></a></li>
															<?php endfor; ?>
															<?php if ($paging->next): ?>
																<li><a href="<?= site_url("cdesa/index/$paging->next/$o")?>" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>
															<?php endif; ?>
															<?php if ($paging->end_link): ?>
																<li><a href="<?= site_url("cdesa/index/$paging->end_link/$o")?>" aria-label="Last"><span aria-hidden="true">Akhir</span></a></li>
															<?php endif; ?>
														</ul>
													</div>
												</div>
